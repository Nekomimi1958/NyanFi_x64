...
void __fastcall TDotNyanDlg::FormShow(TObject *Sender)
{
	...
	Caption = get_MiniPathName(DotNyanName + " の設定", ClientWidth - ScaledInt(48, this), titFont.get());
	...
	TRect &Rect, TOwnerDrawState State)
{
	...
	cv->FillRect(Rect);
	cv->TextOut(Rect.Left + ScaledInt(34, this), yp, vbuf);

	//カラー
	TRect rc = Rect;  rc.Right = rc.Left + ScaledInt(30, this);
	...
	if (cv->Brush->Color!=col_None) {
		...
		cv->FillRect(rc);
		out_Text(cv, rc.Left + ScaledInt(2, this), yp, _T("無効"), get_LabelColor());
	}
}
//---------------------------------------------------------------------------
